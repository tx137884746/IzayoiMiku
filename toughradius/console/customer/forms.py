...
def member_join_form(nodes=[]): 
    return pyforms.Form(
        ...
        pyforms.Textbox("realname", rules.len_of(2,32), description=u"昵称(必填)", required="required",**input_style),
        pyforms.Dropdown("sex", description=u"性别", args=sexopt.items(),required="required", **input_style),
        pyforms.Textbox("age", rules.is_number, description=u"PIN码(必填)", size=3,required="required",**input_style),
        pyforms.Textbox("username", rules.is_alphanum3(6, 32), description=u"账号(必填)", size=32,required="required",**input_style),
        ...
        pyforms.Textbox("idcard", rules.len_of(0,32), description=u"二级密码", **input_style),
        pyforms.Textbox("mobile", rules.len_of(0,32),description=u"QQ（选填）", **input_style),
        pyforms.Textbox("address", description=u"用户预留信息",hr=True, **input_style),
...
password_mail_form =  pyforms.Form(
        pyforms.Textbox("member_name", rules.len_of(1, 64),description=u"请输入账户", required="required",**input_style),
        pyforms.Button("submit",  type="submit", html=u"<b>提交</b>", **button_style),
        title=u"重置密码请求",
...
def member_update_form():
    return pyforms.Form(
        pyforms.Textbox("realname", description=u"昵称",readonly="readonly",**input_style),
        pyforms.Textbox("member_name", description=u"账号", readonly="readonly",**input_style),
